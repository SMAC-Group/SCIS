...
## Linear Processes
In this section we introduce the concept of linear processes. As a matter of fact, the stationary models considered so far can all be represented as linear processes. 
...
\end{equation*}
where $f$ is a frequency. Hence, the PSD is a Fourier transform of the autocovariance function which describes the variance of a time series over frequencies (with respect to lags $h$).
